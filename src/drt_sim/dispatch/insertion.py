"""Insertion 기반 합승 배차 엔진 (온라인 삽입 휴리스틱).

핵심 알고리즘 (greedy insertion heuristic):
  신규 요청 r 에 대해, 후보 차량 v 의 현재 경로에 r 의 픽업/하차 정류점을 가능한 모든
  위치 조합에 삽입해 본다. 각 후보 삽입에 대해
    1) 정원 제약 (어느 시점에도 onboard <= capacity)
    2) 픽업 시간 윈도우 (대기 <= max_wait)
    3) 우회 제약 (탑승 -> 하차 실제 시간 <= 직선시간 * max_detour_factor)
  을 모두 만족하는지 검사하고, **추가되는 총 이동시간(한계비용)이 최소**인 삽입을
  채택한다. 어느 차량에도 실현 가능한 삽입이 없으면 None(거절).

이 프로젝트의 주인공은 분산 시스템이므로 라우팅은 견고한 휴리스틱 수준으로 유지한다.
"""
...
class InsertionPlan:
    ...
    expected_version: int = 0  # 삽입 계산 기준이 된 차량 버전 (낙관적 커밋용)
    ...
    def _feasible(
        ...
        for stop in route:
            ...
            if stop.stop_type == StopType.PICKUP:
                load += stop.party_size  # 각 정류점의 실제 인원으로 정원 검사
