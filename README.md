...
실시간 수요응답형 합승(ride-pooling) 교통 서비스의 배차 엔진을 모사하되, 이 프로젝트의
주인공은 배차 알고리즘이 아니라 **그것을 떠받치는 분산 시스템 설계**다. 지오 샤딩, 노드 간
...
![대시보드: 실제 도로 지도 위 H3 지오 샤드(노드별 색) + 도로 따라 이동하는 차량, 클러스터 토폴로지(리더 배지·헬스), 큐 깊이·배차 지연 차트](assets/dashboard.png)

*실제 도로 타일맵 위에 H3 지오 샤드가 소유 노드별 색으로 칠해지고, 차량이 도로를 따라
이동한다. 오른쪽은 코디네이터+워커 토폴로지(★=리더), 노드별 큐 깊이, 배차 지연 p50/p95/p99.*
