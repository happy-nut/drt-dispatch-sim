...
- **지도 패널** — 운영지역(세종시 10km)을 덮는 **H3 셀을 소유 노드별 색**으로 표시.
...
### 9. 지도 렌더링: SVG 카토그래픽 (vs 타일 슬리피맵)
H3 셀을 실제 위경도에서 평면 km 로 역투영해 **SVG(`go.Scatter`)**로 그린다.
- **왜**: 외부 타일 CDN·WebGL 없이 **오프라인·저사양 노트북에서도 항상 렌더**되고
  스크린샷/GIF 캡처가 된다("단일 커맨드 실행" 요구에 부합). 셀 모양과 소유권 색이
  그대로 보인다.
- **옵션**: 온라인이면 `config.area.map_style` 을 `carto-positron`/`open-street-map`
  으로 바꿔 도로 타일 위에 표시(`go.Scattermap`, MapLibre)할 수 있다.
